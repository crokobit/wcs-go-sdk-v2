...
func (task *uploadPartTask) Run() interface{} {
	...
	var err error
	if extensions != nil {
		output, err = task.wosClient.UploadPart(input, extensions...)
	} else {
		output, err = task.wosClient.UploadPart(input)
	}
	...
	if err == nil {
		...
		return output
	} else if wosError, ok := err.(WosError); ok && wosError.StatusCode >= 400 && wosError.StatusCode < 500 {
		atomic.CompareAndSwapInt32(task.abort, 0, 1)
		doLog(LEVEL_WARN, "Task is aborted, part number is [%d]", task.PartNumber)
	}
	return err
...
func getCheckpointFile(ufc *UploadCheckpoint, uploadFileStat os.FileInfo, input *UploadFileInput, wosClient *WosClient, extensions []extensionOptions) (needCheckpoint bool, err error) {
	...
	if err != nil {
		...
		if ufc.Bucket != "" && ufc.Key != "" && ufc.UploadId != "" {
			_err := abortTask(ufc.Bucket, ufc.Key, ufc.UploadId, wosClient, extensions)
			if _err != nil {
...
func prepareUpload(ufc *UploadCheckpoint, uploadFileStat os.FileInfo, input *UploadFileInput, wosClient *WosClient, extensions []extensionOptions) error {
	...
	var err error
	if extensions != nil {
		output, err = wosClient.InitiateMultipartUpload(initiateInput, extensions...)
	} else {
		output, err = wosClient.InitiateMultipartUpload(initiateInput)
	}
	if err != nil {
		return err
	}
...
func abortTask(bucket, key, uploadID string, wosClient *WosClient, extensions []extensionOptions) error {
	input := &AbortMultipartUploadInput{}
	input.Bucket = bucket
	input.Key = key
	input.UploadId = uploadID
	if extensions != nil {
		_, err := wosClient.AbortMultipartUpload(input, extensions...)
		return err
	}
	_, err := wosClient.AbortMultipartUpload(input)
	return err
}

func handleUploadFileResult(uploadPartError error, ufc *UploadCheckpoint, enableCheckpoint bool, wosClient *WosClient, extensions []extensionOptions) error {
...
func completeParts(ufc *UploadCheckpoint, enableCheckpoint bool, checkpointFilePath string, wosClient *WosClient, encodingType string, extensions []extensionOptions) (output *CompleteMultipartUploadOutput, err error) {
	...
	if extensions != nil {
		completeOutput, err = wosClient.CompleteMultipartUpload(completeInput, extensions...)
	} else {
		completeOutput, err = wosClient.CompleteMultipartUpload(completeInput)
	}
...
func (wosClient WosClient) resumeUpload(input *UploadFileInput, extensions []extensionOptions) (output *CompleteMultipartUploadOutput, err error) {
	...
	if enableCheckpoint {
		needCheckpoint, err = getCheckpointFile(ufc, uploadFileStat, input, &wosClient, extensions)
		if err != nil {
			return nil, err
		}
	}
	if needCheckpoint {
		err = prepareUpload(ufc, uploadFileStat, input, &wosClient, extensions)
		if err != nil {
			return nil, err
		}
	...
	uploadPartError := wosClient.uploadPartConcurrent(ufc, checkpointFilePath, input, extensions)
	err = handleUploadFileResult(uploadPartError, ufc, enableCheckpoint, &wosClient, extensions)
	if err != nil {
		return nil, err
	}

	completeOutput, err := completeParts(ufc, enableCheckpoint, checkpointFilePath, &wosClient, input.EncodingType, extensions)
...
func (wosClient WosClient) uploadPartConcurrent(ufc *UploadCheckpoint, checkpointFilePath string, input *UploadFileInput, extensions []extensionOptions) error {
...
func (dfc *DownloadCheckpoint) isValid(input *DownloadFileInput, output *GetObjectMetadataOutput) bool {
	if dfc.Bucket != input.Bucket || dfc.Key != input.Key || dfc.DownloadFile != input.DownloadFile {
...
func (task *downloadPartTask) Run() interface{} {
	...
	if task.extensions != nil {
		output, err = task.wosClient.GetObject(getObjectInput, task.extensions...)
	} else {
		output, err = task.wosClient.GetObject(getObjectInput)
	}
...
func getObjectInfo(input *DownloadFileInput, wosClient *WosClient, extensions []extensionOptions) (getObjectmetaOutput *GetObjectMetadataOutput, err error) {
	if extensions != nil {
		getObjectmetaOutput, err = wosClient.GetObjectMetadata(&input.GetObjectMetadataInput, extensions...)
	} else {
		getObjectmetaOutput, err = wosClient.GetObjectMetadata(&input.GetObjectMetadataInput)
	}
...
func (wosClient WosClient) resumeDownload(input *DownloadFileInput, extensions []extensionOptions) (output *GetObjectMetadataOutput, err error) {
	getObjectmetaOutput, err := getObjectInfo(input, &wosClient, extensions)
	if err != nil {
		return nil, err
	}
	...
	downloadFileError := wosClient.downloadFileConcurrent(input, dfc, extensions)
...
func (wosClient WosClient) downloadFileConcurrent(input *DownloadFileInput, dfc *DownloadCheckpoint, extensions []extensionOptions) error {
	pool := NewRoutinePool(input.TaskNum, MAX_PART_NUM)
