...
func UrlEncode(value string, chineseOnly bool) string {
	...
	encodeStr := url.QueryEscape(value)
	encodeStr = strings.ReplaceAll(encodeStr, "%2F", "/")
	encodeStr = strings.ReplaceAll(encodeStr, "+", "%20")
	return encodeStr
}

func UrlQueryEncode(value string) string {
	encodeStr := url.QueryEscape(value)
	encodeStr = strings.ReplaceAll(encodeStr, "+", "%20")
...
func getIsWos(isTemporary bool, querys []string, headers map[string][]string) bool {
	isWos := true
	if isTemporary {
...
func GetAuthorization(ak, sk, method, bucketName, objectKey, queryURL string, headers map[string][]string) (ret map[string]string) {
	...
	isWos := getIsWos(isTemporary, querysResult, headers)
	headers = copyHeaders(headers)
	pathStyle := false
	if receviedHost, ok := headers[HEADER_HOST]; ok && len(receviedHost) > 0 && !strings.HasPrefix(receviedHost[0], bucketName+".") {
		pathStyle = true
	...
	if isTemporary {
		return getTemporaryAuthorization(ak, sk, method, bucketName, objectKey, signature, conf, params, headers, isWos)
	}
	...
	if signature == "v4" {
		...
		ret = v4Auth(ak, sk, region, method, canonicalizedURL, parsedRequestURL.RawQuery, _headers, isWos)

		if isWos {
			ret[HEADER_AUTH_CAMEL] = fmt.Sprintf("%s Credential=%s,SignedHeaders=%s,Signature=%s", V4_WOS_HASH_PREFIX, ret["Credential"], ret["SignedHeaders"], ret["Signature"])
		} else {
...
func getTemporaryAuthorization(ak, sk, method, bucketName, objectKey, signature string, conf *config, params map[string]string,
	headers map[string][]string, isWos bool) (ret map[string]string) {
	...
	if signature == "v2" {
		...
		_, scope := getCredential(ak, region, shortDate, isWos)
		...
		stringToSign := getV4StringToSign(method, canonicalizedURL, parsedRequestURL.RawQuery, scope, longDate, UNSIGNED_PAYLOAD, strings.Split(signedHeaders, ";"), headers, isWos)
		ret[PARAM_SIGNATURE_AMZ_CAMEL] = UrlEncode(getSignature(stringToSign, sk, region, shortDate, isWos), false)
	} else if signature == "v4" && isWos {
		conf.signature = SignatureWos

		longDate, ok := params[PARAM_DATE_WOS_CAMEL]
		if !ok {
			longDate = params[HEADER_DATE_WOS]
		}
		shortDate := longDate[:8]

		credential, ok := params[PARAM_CREDENTIAL_WOS_CAMEL]
		if !ok {
			credential = params[strings.ToLower(PARAM_CREDENTIAL_WOS_CAMEL)]
		}
		...
		expires, ok := params[PARAM_EXPIRES_WOS_CAMEL]
		if !ok {
			expires = params[strings.ToLower(PARAM_EXPIRES_WOS_CAMEL)]
		}

		signedHeaders, ok := params[PARAM_SIGNEDHEADERS_WOS_CAMEL]
		if !ok {
			signedHeaders = params[strings.ToLower(PARAM_SIGNEDHEADERS_WOS_CAMEL)]
		}

		algorithm, ok := params[PARAM_ALGORITHM_WOS_CAMEL]
		if !ok {
			algorithm = params[strings.ToLower(PARAM_ALGORITHM_WOS_CAMEL)]
		}

		if _, ok := params[PARAM_SIGNATURE_WOS_CAMEL]; ok {
			delete(params, PARAM_SIGNATURE_WOS_CAMEL)
		} else if _, ok := params[strings.ToLower(PARAM_SIGNATURE_WOS_CAMEL)]; ok {
			delete(params, strings.ToLower(PARAM_SIGNATURE_WOS_CAMEL))
		}

		ret = make(map[string]string, 6)
		ret[PARAM_ALGORITHM_WOS_CAMEL] = algorithm
		ret[PARAM_CREDENTIAL_WOS_CAMEL] = credential
		ret[PARAM_DATE_WOS_CAMEL] = longDate
		ret[PARAM_EXPIRES_WOS_CAMEL] = expires
		ret[PARAM_SIGNEDHEADERS_WOS_CAMEL] = signedHeaders
		...
		stringToSign := getV4StringToSign(method, canonicalizedURL, parsedRequestURL.RawQuery, scope, longDate, UNSIGNED_PAYLOAD, strings.Split(signedHeaders, ";"), headers, isWos)
		ret[PARAM_SIGNATURE_WOS_CAMEL] = UrlEncode(getSignature(stringToSign, sk, region, shortDate, isWos), false)
	}
	return
}
