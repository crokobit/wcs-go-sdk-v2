...
// WithSslVerifyAndPemCerts is a configurer for WosClient to set conf.sslVerify and conf.pemCerts.
...
// WithHeaderTimeout is a configurer for WosClient to set the timeout period of obtaining the response headers.
...
// WithProxyUrl is a configurer for WosClient to set HTTP proxy.
...
// WithMaxConnections is a configurer for WosClient to set the maximum number of idle HTTP connections.
...
// WithPathStyle is a configurer for WosClient.
...
// WithSignature is a configurer for WosClient.
...
// WithRegion is a configurer for WosClient.
...
// WithConnectTimeout is a configurer for WosClient to set timeout period for establishing
...
// WithSocketTimeout is a configurer for WosClient to set the timeout duration for transmitting data at
...
// WithIdleConnTimeout is a configurer for WosClient to set the timeout period of an idle HTTP connection
...
// WithMaxRetryCount is a configurer for WosClient to set the maximum number of retries when an HTTP/HTTPS connection is abnormal.
...
// WithHttpTransport is a configurer for WosClient to set the customized http Transport.
...
// WithRequestContext is a configurer for WosClient to set the context for each HTTP request.
...
// WithCustomDomainName is a configurer for WosClient.
...
// WithMaxRedirectCount is a configurer for WosClient to set the maximum number of times that the request is redirected.
...
// WithUserAgent is a configurer for WosClient to set the User-Agent.
...
// WithEnableCompression is a configurer for WosClient to set the Transport.DisableCompression.
...
func (conf *config) formatUrls(bucketName, objectKey string, params map[string]string, escape bool) (requestURL string, canonicalizedURL string) {
	...
	for index, key := range keys {
		...
		if conf.signature == SignatureV2 {
			...
			//isWos := conf.signature == SignatureWos
			//if isWos {
			//	prefixHeader = HEADER_PREFIX_WOS
			//}
			...
			requestURL += "=" + UrlQueryEncode(_value)
		}
	}
	return
}
